...
class connect
{
	...
	{
		$dsn='mysql:host=localhost;dbname=pet_store';
		$user='root';
		$pass=''; 
...
//	Lấy nhiều rows
	public function getList($select)//select * from hanghoa order by mahh limit 12
	{
		$results=$this->db->query($select);
		// echo $results;
		return($results);
	...
	public function getListP($select)//select * from hanghoa order by mahh limit 12
	{
...
//	https://viblo.asia/p/cai-dat-ung-dung-php-thuan-su-dung-mvc-va-oop-4P856aA3lY3
	...
	{
		$results=$this->db->exec($query);//dùng cho lện insert
		// echo $results;
		return($results);
	}
